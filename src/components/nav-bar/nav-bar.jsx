  ...
  return (
    <nav ref={navBarRef} className="flex items-center justify-between flex-wrap bg-black p-3 z-50">
      <Link to="/" className="flex items-center flex-shrink-0 text-white hover:text-red-700 mr-6">
        <img src={logo} className="fill-current h-12 mr-2" alt="Makery logo" />
        <span className="font-semibold text-xl tracking-tight">GT Makery</span>
      </Link>
      ...
      <div className={`w-full ${responsive ? "block z-50" : "hidden"} flex-grow md:flex md:items-center md:w-auto absolute md:relative top-full md:top-0 bg-black -ml-3 md:ml-0 p-3 md:p-0 pt-0`}>
        <div className="text-sm md:flex-grow">
          ...
          <Link to="/bluejeans" className="block mt-4 mb-4 md:mb-0 md:inline-block md:mt-0 text-white hover:text-red-700 mr-4">
            Bluejeans
          </Link>
          <Link to="/youtube" className="block mt-4 mb-4 md:mb-0 md:inline-block md:mt-0 text-white hover:text-red-700 mr-4">
            Youtube
          </Link>
        </div>
      </div>
